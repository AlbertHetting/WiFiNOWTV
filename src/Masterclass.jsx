...
export default function MasterClass() {
  const VideoInfo = dummydata
    .filter((video) => video.Tag === "Masterclass")
    .slice(0, 2); // Tag kun 2 videoer

  const [searchQuery, setSearchQuery] = useState("");

  const latestVendorVideosCat = dummydata
    .filter((video) => video.Tag === "Masterclass") // tag kun Masterclass
    .sort(
  ...
  const filteredVideos = dummydata.filter((video) => video.Tag === "Masterclass"); //Kun Masterclass videos søges igennem!
  (
          ...
          <div className="FeaturedVideos">
            {VideoInfo.map((video) => (
              <VideoCardLarge
                key={
                  video.id
                } /* React skal burge et unikt ID for hvert objekt i et map */
                id={video.id}
                title={video.title}
                date={video.date}
            ...
            {searchQuery ? ( //Conditional rendering fra react (if else statement) hvis der er en search query så vis search video con ellers vis video category con
              ...
              <section className="VideoCategoryCon">
                <div className="WeeklyNewsCat">
                  <div className="VideoGridFront">
                    {/*  Mpa function */}
                    {latestVendorVideosCat.map((video) => (
                      <VideoCard
                        key={
                          video.id
                        } /* Unikt ID for mapped items (skal react altid bruge!)  */
