...
export default function WeeklyNews() {
  const VideoInfo = dummydata.filter((video) => video.id).slice(0, 2); // Tag kun 2 videoer

  const [searchQuery, setSearchQuery] = useState("");

  const latestWeeklyVideosCat = dummydata
    .filter((video) => video.id) // tag alle videoer!
    .sort(
      (a, b) => parseInt(b["release-number"]) - parseInt(a["release-number"]),
    ) // Sortering fra højeste til laveste (dato)
    .slice(2, 60); // skip first 2 video, derefter tag op til nummer 20 video
          ...
          <div className="FeaturedVideos">
            {VideoInfo.map((video) => (
              <VideoCardLarge
                key={
                  video.id
                } /* React skal ahve et unikt ID for hvert obejekt */
                id={video.id}
                title={video.title}
                date={video.date}
